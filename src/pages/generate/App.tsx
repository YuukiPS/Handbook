...
const App: React.FC = (): JSX.Element => {
	...
	const selectFolder = async (title: string, setPath: (path: string) => void) => {
		...
		const selectedPath = typeof result === 'string' ? result : result?.displayName
		if (!selectedPath) {
			toast({
				title: 'Folder Select',
	...
	const selectTextMapPath = async () => {
		try {
			const selectedPath = await selectFolder('Select a directory to where Text Map', setSelectedTextMapPath)
			...
			<div className='max-w-4xl mx-auto'>
				...
				<div className='bg-white dark:bg-gray-800 rounded-lg shadow-md p-6'>
					<div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
						...
						<div>
							<div>
								<Label className='mb-2 block'>Type of Game</Label>
