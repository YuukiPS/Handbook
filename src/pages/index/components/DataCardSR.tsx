    ...
    return (
        <div className="flex-shrink-0">
            <LazyLoadImage
                ...
                className={`w-32 rounded-lg max-sm:ml-3 sm:ml-3 md:ml-0 ${convertNumberToText(
                    item.rarity || 1
                )}`}
                effect="opacity"
                onError={(e) => {
...
const DataCardSR: React.FC<DataCardSRProps> = ({
            ...
            toast({
                ...
                action: (
                    <ToastAction
                        ...
                        altText={tToast(
                            "apply_command_no_configured.action.text"
                        )}
                    >
    ...
    const handleModalExecute = () => {
        ...
        const userValues = state.args.reduce<Record<string, string>>(
            (obj, arg) => {
        ...
        const generateCommand = YuukiPS.generateResultCommand(
            state.command,
            userValues
        );
        setStateApp((prev) => ({
            ...
            output: [
                ...prev.output,
                tOutput("executing_command", { command: generateCommand }),
            ],
        }));
            ...
            <Dialog open={state.openModal}>
                <DialogContent>
                    ...
                    {state.args.map((arg) => (
                        <>
                            <Label
                                key={`apply-the-command-args-${arg.replace(
                                    / /,
                                    ""
                                )}`}
                            >
