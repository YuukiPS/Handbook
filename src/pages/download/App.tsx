...
const Download: React.FC = (): JSX.Element => {
	...
	const handleSelectOutputPath = async () => {
		...
		if (!folderSelector) {
			toast({
				title: 'Not Supported',
				description: 'Platform does not support folder selection, or permission was denied on Android.',
				variant: 'destructive',
			})
			return null
		}

		const folder = await folderSelector('Select Output Path').then((res) => {
	...
	const handleDownload = async () => {
		...
		try {
			...
			setOutputLog((prev) => [...prev, { log_level: 'error', message: `Error: ${error}` }])
		} finally {
	...
	return (
		<div className='container mx-auto p-4 space-y-8'>
			<h1 className='text-2xl md:text-3xl font-bold text-center'>Resource Download</h1>
			<div className='bg-card text-card-foreground rounded-lg shadow-md p-4 md:p-6 space-y-4 max-w-3xl mx-auto'>
				<div className='space-y-6'>
					<div className='md:flex md:space-x-4 space-y-6 md:space-y-0'>
						<div className='md:w-1/2'>
							<h3 className='text-lg font-semibold mb-2'>Genshin Impact</h3>
							<div className='space-y-2'>
								<Label htmlFor='textmap-select-gi'>Select TextMaps</Label>
								<MultiSelect
							...
							<div className='space-y-2 mt-2'>
								<Label htmlFor='excel-select-gi'>Select Excel Files</Label>
								<MultiSelect
									id='excel-select-gi'
						...
						<div className='md:w-1/2'>
							<h3 className='text-lg font-semibold mb-2'>Star Rail</h3>
							<div className='space-y-2'>
								<Label htmlFor='textmap-select-sr'>Select TextMaps</Label>
								<MultiSelect
							...
							<div className='space-y-2 mt-2'>
								<Label htmlFor='excel-select-sr'>Select Excel Files</Label>
								<MultiSelect
									id='excel-select-sr'
					...
					<div className='w-full flex justify-center'>
						<div className='w-full'>
							...
							<Button onClick={handleSelectOutputPath} variant='outline' className='w-full justify-start'>
				...
				{downloadProgress > 0 && <DownloadProgress progress={downloadProgress} speed={downloadSpeed} />}
			</div>
			<Output
				outputLog={outputLog}
...
const DownloadProgress: React.FC<{ progress: number; speed: number }> = ({ progress, speed }) => (
	<div className='space-y-2'>
