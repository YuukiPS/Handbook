...
const SelectAccount: React.FC<SelectAccountProps> = memo(({ listServer, handleSelectAccount }) => (
	<DropdownMenu>
		...
		<DropdownMenuContent>
			...
			<DropdownMenuRadioGroup onValueChange={handleSelectAccount}>
				{listServer.map((player) => (
					...
					>
						{player.server.title} | {player.player.data?.nickname} | {player.player.data?.uid}
					</DropdownMenuRadioItem>
				))}
			</DropdownMenuRadioGroup>
		</DropdownMenuContent>
